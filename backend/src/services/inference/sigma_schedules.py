...
注:diffusers pipe(sigmas=...)期望**不含末尾 0**的 sigma 序列(它自己 append 0);
ComfyUI 的 scheduler 返回**含末尾 0**(steps+1 个)。本模块返回**含末尾 0**(对齐 ComfyUI
ground truth 便于测试),注入 pipe 前由调用方去掉末尾 0(见 image_modular 注入点)。
"""
...
def _time_snr_shift(shift: float, t: np.ndarray) -> np.ndarray:
...
class FlowModelSampling:
    ...
    def __init__(self, shift: float = 3.0, multiplier: float = 1.0, timesteps: int = 1000):
        ...
        ts = (np.arange(1, timesteps + 1, dtype=np.float64) / timesteps) * self.multiplier
        self.sigmas = _time_snr_shift(self.shift, ts / self.multiplier)  # (1000,)
    ...
    def sigma(self, t: float) -> float:
        return float(_time_snr_shift(self.shift, t / self.multiplier))
...
def _normal(ms: FlowModelSampling, steps: int, sgm: bool = False) -> list[float]:
    ...
    sigs = [ms.sigma(float(t)) for t in timesteps]
    if append_zero:
        sigs += [0.0]
    return sigs
...
def _karras(ms: FlowModelSampling, steps: int, rho: float = 7.0) -> list[float]:
    ...
    return [float(x) for x in sigs] + [0.0]


def _exponential(ms: FlowModelSampling, steps: int) -> list[float]:
    sigma_min, sigma_max = ms.sigma_min, ms.sigma_max
    sigs = np.exp(np.linspace(math.log(sigma_max), math.log(sigma_min), steps))
    return [float(x) for x in sigs] + [0.0]
...
def _beta(ms: FlowModelSampling, steps: int, alpha: float = 0.6, beta: float = 0.6) -> list[float]:
    ...
    ts = np.rint(stats.beta.ppf(ts, alpha, beta) * total)
    sigs: list[float] = []
    last_t = -1
    for t in ts:
        if t != last_t:
...
def _linear_quadratic(ms: FlowModelSampling, steps: int,
    ...
    return [float(x * ms.sigma_max) for x in sched]


def _kl_optimal(ms: FlowModelSampling, steps: int) -> list[float]:
    n = steps
    adj = np.arange(n, dtype=np.float64) / (n - 1)
    sigmas = np.zeros(n + 1)
    sigmas[:-1] = np.tan(adj * math.atan(ms.sigma_min) + (1 - adj) * math.atan(ms.sigma_max))
    return [float(x) for x in sigmas]


# scheduler 名 → 计算函数。normal/karras/exponential/beta 仍可走 diffusers use_*_sigmas
# (image_modular 现状),但这里也提供以便统一/对齐测试。
SIGMA_SCHEDULES = {
