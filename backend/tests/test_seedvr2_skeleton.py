...
def test_vendored_dirs_present():
    """vendoring 完整:5 核心目录 + data + LICENSE。不带 interfaces(ComfyUI 桥层)。"""
    for d in ["core", "optimization", "common", "utils", "models", "data"]:
        assert (_VENDOR / d).is_dir(), f"vendored 缺 {d}/"
    assert (_VENDOR / "LICENSE_SEEDVR2").exists(), "缺 Apache LICENSE 署名"
    assert not (_VENDOR / "interfaces").exists(), "interfaces/ 是 ComfyUI 桥层,不该 vendoring"


def test_vendored_core_has_inference_entrypoints():
    """vendored core 有 prepare_runner + 4 阶段函数(adapter PR-2 要调的)。"""
    gu = (_VENDOR / "core/generation_utils.py").read_text()
    gp = (_VENDOR / "core/generation_phases.py").read_text()
