...
class _FakeMemoryProvider(MemoryProvider):
    ...
    async def add_entries(self, *, owner_key_id, entries, context_key=None):
        if not entries:
            return []
        if len(entries) > 100:
    ...
    async def prefetch(self, *, owner_key_id, query, limit=10, context_key=None):
        if self._simulate_fail:
            return []  # best-effort swallow
        results = [r for r in self._store if r["api_key_id"] == owner_key_id]
        if context_key:
...
class AbstractMemoryProviderTests:
    ...
    async def test_add_entries_batch_over_100_raises(self, provider):
        ...
        with pytest.raises(MemoryProviderClientError):
            await provider.add_entries(owner_key_id=1, entries=big)
    ...
    async def test_prefetch_returns_matching(self, provider):
        ...
        results = await provider.prefetch(owner_key_id=1, query="Tokyo", limit=10)
        assert len(results) == 1
        assert "Tokyo" in results[0]["content"]

    @pytest.mark.asyncio
    async def test_cross_owner_isolation(self, provider):
        await provider.add_entries(
            owner_key_id=1,
            entries=[{"category": "preference", "content": "foo", "context_key": None}],
        )
        results = await provider.prefetch(owner_key_id=2, query="foo")
        assert results == []
    ...
    async def test_context_key_filters_prefetch(self, provider):
        ...
        r = await provider.prefetch(owner_key_id=1, query="project", context_key="alpha")
        assert len(r) == 1
    ...
    async def test_prefetch_swallows_internal_error(self, provider):
        ...
        if hasattr(provider, "_simulate_fail"):
            ...
            results = await provider.prefetch(owner_key_id=1, query="anything")
            assert results == []
