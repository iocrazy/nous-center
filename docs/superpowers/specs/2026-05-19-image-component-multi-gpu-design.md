...
- Flux2-bf16 三组件分卡(unet→cuda:1, clip→cuda:0, vae→cuda:2)走通,出图 ≤ 35s,**且 SSIM 跟单 device baseline > 0.99**(自写采样数学正确性)
  - **(PR-4 真模型 smoke 修正,2026-05-20)**:`SSIM > 0.99` 仅在**同 GPU 架构**下成立。实测自写 ImageSampler **同卡 vs 官方 Flux2KleinPipeline = SSIM 1.0000(逐像素一致)**,数学完全正确;但本机 Pro 6000(Blackwell)+ 双 3090(Ampere)是**混合架构**,把 clip/vae 放 3090、transformer 放 Pro 6000 时,bf16 在两种架构上的 forward 有硬件固有微小方差,**跨架构 SSIM ≈ 0.9831**(视觉无差异,二跑确定性 SSIM=1.0000)。结论:成功标准修正为「**同架构跨卡 > 0.99;跨架构跨卡 ≈ 0.98 可接受**(硬件固有,非代码缺陷)」。
