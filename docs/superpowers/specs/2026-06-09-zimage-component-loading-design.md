...
### PR-3:GGUF 文本编码器(可选,评估后定)
- `CLIPLoaderGGUF` 等价:GGUF 量化文本编码器反量化加载(Z-Image-Engineer)。nous 现无 GGUF text encoder 路径。
- 工作量大(GGUF dequant + 装配),且需真文件;**先评估**,PR-1/2 落地后按用户需要再排。
...
执行顺序:PR-1(采样器)→ PR-2(分开载入)。PR-3(GGUF)评估后定。每 PR 独立分支 + CI 绿 + 引擎改真机 smoke。
