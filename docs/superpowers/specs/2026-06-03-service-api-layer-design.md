...
## 4. PR 拆分(大改,顺序递进,各自真机验)
- **PR-1 类型化 schema**:发布生成 per-service JSON-Schema + `GET /services/{name}/openapi.json` + 调用期校验。纯加,不破坏现有调用。
- **PR-2 统一 Prediction + 同步/异步端点**:`POST /services/{name}/predictions`(Prefer 头切)+ `GET /predictions/{id}`;泛化 ExecutionTask→Prediction;**修 workflow 501**。
- **PR-3 进度流 + webhook**:SSE/ws per prediction(复用 node 进度事件)+ webhook(Cog 形)。
- **PR-4 输出交付契约**:prediction.output 图→data-URI / 签名 URL+TTL(复用 image_output_storage),删节点 URL widget。
- **PR-5 compat 退化成 shim + auth/quota 收敛**:OpenAI/Ollama/Anthropic 翻译到统一路径,校验收一处。
- (每 PR 保持对外向后兼容:老 `/v1/instances/{id}/run` + 老 key 路径过渡期并存,标 deprecated。)

## 5. 风险 / 坑
- **大改对外契约 + 执行路径** —— 必须真机端到端验(发布→拿 schema→同步调→异步调+轮询→进度→取图)。
- 向后兼容:现有 key/instance/`/run` 路径过渡期并存,不能一刀切(已有外部调用方)。
- M:N auth/quota 收敛要小心别开新洞(explorer 已指出现状分散,收敛是机会也是风险点)。
