...
echo "   真正把 persistence mode 打开的是下面的 nous-engine-gpu-guard(-pm 1))"
systemctl enable --now nvidia-persistenced.service || \
  echo "   ⚠️ nvidia-persistenced enable 失败(驱动未装该 unit?跳过)"

echo ">> 装 + enable nous-engine-gpu-guard(开机强制 persistence mode)"
...
# oneshot 强制到位(-pm 1),每次启动生效,不再"缓解没上机"。
chmod +x "$SD/nous-gpu-guard.sh"
install -m 0644 "$SD/nous-engine-gpu-guard.service" /etc/systemd/system/nous-engine-gpu-guard.service
systemctl daemon-reload
systemctl enable --now nous-engine-gpu-guard.service && \
  echo "   ✅ nous-engine-gpu-guard 已 enable + 立即执行" || \
  echo "   ⚠️ nous-engine-gpu-guard 启动失败,看 journalctl -u nous-engine-gpu-guard"

echo
echo "✅ 完成。modprobe 改动需**重启**生效;persistence mode 已即时应用(并随开机强制)。"
echo "   建议配合冷断电(断 AC 2-3min)重启,既应用本缓解,又复位掉 PRO 6000 的"
echo "   fullchip-reset 僵死。核验:nvidia-smi --query-gpu=index,persistence_mode --format=csv"
